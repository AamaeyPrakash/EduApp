...
const MathText = ({ content }) => {
  ...
  const processedContent = parts.reduce((acc, part, index) => {
    if (part.startsWith('$$') && part.endsWith('$$')) {
      ...
      acc.push(
        <MathView key={index} math={math} style={styles.blockMath} />
      );
    } else if (part.startsWith('$') && part.endsWith('$')) {
      // Display inline math - preserve exact LaTeX
      const math = part.slice(1, -1).trim();
      acc.push(
        <MathView key={index} math={math} style={styles.inlineMath} />
      );
    } else if (part) {
      ...
      lines.forEach((line, lineIndex) => {
        if (lineIndex > 0) {
          acc.push(
            <View key={`br-${index}-${lineIndex}`} style={styles.lineBreak} />
          );
        }
        if (line) {
          acc.push(
  ...
  return <View style={styles.mathTextContainer}>{processedContent}</View>;
};
